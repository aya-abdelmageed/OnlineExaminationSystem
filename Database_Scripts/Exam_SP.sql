...
AS
BEGIN
    IF EXISTS (SELECT @EXAM_ID FROM Exam WHERE @EXAM_ID = Exam.Exam_ID)
    AND EXISTS (SELECT @QUESTION_ID FROM Questions WHERE @Question_ID = Questions.Question_ID)
        INSERT INTO Exam_Questions (Exam_ID, Question_ID, Points_Edit)
        VALUES(@EXAM_ID, @QUESTION_ID, @POINTS_EDIT);
    ELSE
        SELECT 'Invalid Exam_ID or Question_ID';
END
GO
...
-- update 
GO
CREATE PROC EXAM_QUESTIONS_UPDATE @EXAM_ID INT, @Q_ID INT, @POINTS_EDIT INT
WITH ENCRYPTION
AS
BEGIN
    IF EXISTS (SELECT 1 FROM Exam_Questions WHERE @EXAM_ID = Exam_Questions.Exam_ID AND @Q_ID = Exam_Questions.Question_ID)
        UPDATE Exam_Questions
	    SET 
	       Points_Edit = @POINTS_EDIT
	    WHERE Exam_Questions.Question_ID = @Q_ID AND Exam_Questions.Exam_ID = @EXAM_ID 
	ELSE
		  SELECT 'INVAILD EXAM OR QUESTION ID'
END
-- DELETION
GO
CREATE PROC EXAM_QUESTION_DELETION @E_ID INT, @Q_ID INT
WITH ENCRYPTION
AS
BEGIN 
    IF EXISTS (SELECT 1 FROM Exam_Questions WHERE @E_ID = Exam_Questions.Exam_ID AND @Q_ID = Exam_Questions.Question_ID)
...
BEGIN
    DECLARE @Grade INT;
    IF EXISTS (SELECT @EXAM_ID FROM Exam WHERE @EXAM_ID = Exam.Exam_ID)
	AND EXISTS(SELECT @Student_ID FROM Student WHERE @Student_ID = Student.Student_ID)
	BEGIN
	...
	END
	ELSE
	  SELECT ('INVAILD STUDENT ID OR EXAM ID')
END;
GO
...
BEGIN
    ...
    BEGIN
        ...
        SELECT 'Record successfully inserted.';
    END
    ELSE
    BEGIN
        SELECT 'Record already exists. Insertion failed.';
    END
END;
GO
...
BEGIN
    ...
    BEGIN
     IF EXISTS (SELECT 1 
        ...
        BEGIN
            SELECT 'Update failed: A record with the new Exam_ID already exists for the given Track_ID.';
        END
        ELSE
        BEGIN
            UPDATE Exam_Track
    ...
    ELSE
    BEGIN
        SELECT 'Update failed: Record not found.';
    END
END;
GO
...
BEGIN
    ...
    SELECT Student_ID, Exam_ID, Question_ID, Answer, Score
    FROM Exam_Question_Student
    WHERE (@S_ID IS NULL OR Student_ID = @S_ID)
      AND (@E_ID IS NULL OR Exam_ID = @E_ID)
...
BEGIN
    ...
    BEGIN
        ...
        ELSE
        BEGIN
            SELECT 'No new answer provided. No update to score or answer.';
        END
    END
    ELSE
    BEGIN
        SELECT 'Invalid Student_ID, Exam_ID, or Question_ID. Update failed.';
    END
END;
GO
...
BEGIN
    ...
    BEGIN
        ...
        SELECT 'Record successfully deleted.';
    END
    ELSE
    BEGIN
        SELECT 'Record not found. Deletion failed.';
    END
END;
GO
...
-- Exam generation
GO
ALTER PROC EXAM_GENERATION @CRS_NAME VARCHAR(255),@TF_NUM INT,@MCQ_NUM INT,@DTE DATE,@ST TIME,@ET TIME,@INS_ID INT = NULL
WITH ENCRYPTION
AS
BEGIN
   DECLARE @Crs_ID INT;  
